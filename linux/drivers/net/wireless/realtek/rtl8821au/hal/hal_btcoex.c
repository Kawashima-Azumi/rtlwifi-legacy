...
)
{
	...
	rtw_merge_string(rtw_phy_file_path_abs, PATH_LENGTH_MAX, rtw_phy_file_path, pFileName);

	if (rtw_is_file_readable(rtw_phy_file_path_abs) == _TRUE)
	{
		rlen = rtw_retrieve_from_file(rtw_phy_file_path_abs, pHalData->para_file_buf, MAX_PARA_FILE_BUF_LEN);
		if (rlen > 0)
		{
			rtStatus = _SUCCESS;
		}
